...
docker build -t carsimulatorbuild .
#docker run --rm -v "$(pwd):/proj" -w /proj carsimulatorbuild build-wrapper-linux-x86-64 --out-dir bw-output make clean coverage_build
docker run --rm -v "$(pwd):/proj" -w /proj carsimulatorbuild build-wrapper-linux-x86-64 --out-dir bw-output make clean all
docker run --rm -v "$(pwd):/proj" -w /proj carsimulatorbuild ./build_cov/runTest --gtest_output="xml:./testOutput.xml"
#docker run --rm -v "$(pwd):/proj" -w /proj carsimulatorbuild make coverage
docker run --rm -v "$(pwd):/proj" -w /proj carsimulatorbuild sonar-scanner -Dsonar.login="$1" $SONAR_BRANCH
